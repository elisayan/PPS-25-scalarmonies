...
object FieldsScoring:
  ...
  def compute(
               board: PersonalBoard,
               buildGroup: (Set[Coordinate], Set[Coordinate]) => (Set[Coordinate], Set[Coordinate])
             ): Score =
...
object MountainsScoring:
  ...
  def compute(
    ...
    val mountainHeights = board.cells.collect {
      case (coord, cell) if cell.topToken.contains(TerrainToken.Mountain) => coord -> cell.height
    }
...
object BuildingsScoring:
  ...
  def compute(board: PersonalBoard): Score =
    val allBuildings: Set[Coordinate] = board.cells.collect {
      case (coord, cell) if cell.topToken.contains(TerrainToken.Building) => coord
    }.toSet
    ...
    def isValidBuilding(coord: Coordinate): Boolean =
      ...
      neighbourTerrains.contains(TerrainToken.Building) && neighbourTerrains.size >= 3
...
object ForestsScoring:
  ...
  def compute(board: PersonalBoard): Score =
    val treeHeights: Map[Coordinate, Int] = board.cells.collect {
      case (coord, cell) if cell.topToken.contains(TerrainToken.Forest) => coord -> cell.height
    }
