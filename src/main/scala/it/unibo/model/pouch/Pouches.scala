...
object Pouches:
  ...
  object Pouch:
     ...
     * @param tokens initial tokens to insert.
     * @param seed random seed for deterministic testing.
     * @return a new [[Pouch]] with shuffled tokens.
     */
    def apply(tokens: List[TerrainToken], seed: Long = Random.nextLong()): Pouch =
      Random(seed).shuffle(tokens)
    /**
     * Generates the standard initial pouch for a match,
     * containing the exact token distribution (120 tokens total).
     * @param seed random seed for shuffling.
     * @return the starting [[Pouch]].
     */
    ...
    extension (p: Pouch)
       ...
       * @param amount number of tokens to draw.
       * @return a tuple containing the drawn tokens and the updated [[Pouch]].
       */
