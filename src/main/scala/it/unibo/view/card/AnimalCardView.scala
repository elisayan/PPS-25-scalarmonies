...
object AnimalCardView:
  ...
  def apply(card: AnimalCard, cardWidth: Double = 180.0, cardHeight: Double = 280.0): Node =
    ...
    val hexShape  = s"M ${hexWidth * Layout.HexPoint1} 0 L ${hexWidth * Layout.HexPoint2} 0 L $hexWidth ${hexHeight / 2.0} L ${hexWidth * Layout.HexPoint2} $hexHeight L ${hexWidth * Layout.HexPoint1} $hexHeight L 0 ${hexHeight / 2.0} Z"
    ...
    new BorderPane:
      ...
      center = new VBox:
        ...
        children = Seq(
          ...
          renderHabitat(card.habitat, habW, habH, hexWidth, hexHeight, hexShape, deltaX, deltaY)
        )
      right = renderScoringTrack(card, cubeSize, trackPadTop, paddingSize, mainFont)

  private def renderImage(imageId: String, imgSize: Double, fontSize: Double): Node =
  ...
  private def renderHabitat(habitat: Habitat, habW: Double, habH: Double, hexW: Double, hexH: Double, hexShape: String, dX: Double, dY: Double): Node =
    new Pane:
      prefWidth = habW
      prefHeight = habH
      style = "-fx-background-color: rgba(255, 255, 255, 0.5); -fx-border-color: #555; -fx-border-style: dashed; -fx-background-radius: 5;"
      val sortedReqs = habitat.requirements.sortBy(req => toCartesianY(req.offset, dY))
      children = sortedReqs.map: req =>
        new StackPane:
          ...
          val tokens = (0 until req.height).map: i =>
            ...
            val hexColor = if req.terrain == TerrainToken.Forest && i < req.height - 1 then
              "#8b5a2b"
            else
              tokenToCssHex(req.terrain)
            new Region:
          ...
          val animalCube = if req.offset == Coordinate(0, 0) then
            ...
            Seq(new Region:
              ...
              maxWidth = cSize
              maxHeight = cSize
              style = "-fx-background-color: #e6b981; -fx-border-color: #5e3a18; -fx-border-width: 2px; -fx-background-radius: 3;"
            )
          else Seq.empty
          children = tokens ++ animalCube

  private def renderScoringTrack(card: AnimalCard, cubeSize: Double, padTop: Double, padLeft: Double, fontSize: Double): Node =
    new VBox:
        ...
        new VBox:
          ...
          children = Seq(
            ...
            new Label(pointValue.toString):
              style = s"-fx-font-weight: bold; -fx-text-fill: #333333; -fx-font-size: ${fontSize}px;"
          )
  ...
  private def toCartesianX(coord: Coordinate, dX: Double): Double = coord.x * (dX / 2.0)
  private def toCartesianY(coord: Coordinate, dY: Double): Double = -coord.y * (dY / 2.0)