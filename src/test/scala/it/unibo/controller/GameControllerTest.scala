...
class GameControllerTest extends AnyFlatSpec with Matchers:
  ...
  private val players = List(Player(1, PersonalBoard(SideA)), Player(2, PersonalBoard(SideA)))
  ...
  it should "place token through controller" in:
    ...
    val coord = TokenValidator.validPositions(token, controller.currentModel.currentPlayer.board).head
