...
class HabitatMatcherTest extends AnyFunSuite:
  ...
  test("isMatch deve ritornare true se la plancia soddisfa l'habitat altrimenti falso"):
    ...
    HabitatMatcher.isMatch(boardWithFreeTarget, Coordinate(10, 10), testHabitat) shouldBe false
  ...
  test("findMatches deve ritornare Set vuoto se l'habitat è impossibile"):
    ...
    HabitatMatcher.findMatches(boardWithFreeTarget, impossibleHabitat) shouldBe empty

  test("findMatches deve ritornare Set vuoto se la cella bersaglio è occupata"):
    HabitatMatcher.findMatches(boardWithOccupiedTarget, testHabitat) shouldBe empty
  ...
  test("findMatches test su SideA (rotazioni, traslazioni e altezze errate)"):
    ...
    val largeBoard = PersonalBoard(SideA)
      ...
      .placeToken(Mountain, Coordinate(2, -1)).placeToken(Mountain, Coordinate(2, -1))
