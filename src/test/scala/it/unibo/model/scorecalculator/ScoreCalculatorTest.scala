...
class ScoreCalculatorTest extends AnyFlatSpec with Matchers:
  ...
  it should "correctly isolate and calculate score from fields" in:
    ...
    val (total, details) = calculator.calculateDetailedScore(f3.get)
  ...
  it should "correctly calculate score from buildings" in:
    ...
    val (total, details) = calculator.calculateDetailedScore(b6.get)
    ...
    val (tot, det) = calculator.calculateDetailedScore(b10.get)
  ...
  it should "correctly calculate score from forests" in:
    ...
    val forestBoard2 = createMockBoard()
    ...
    val (tot, dets) = calculator.calculateDetailedScore(f6.get)
  ...
  it should "correctly calculate score for longest river of water tokens" in:
    ...
    val (total, details) = calculator.calculateDetailedScore(w6.get)
