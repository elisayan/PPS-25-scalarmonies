...
object GameViewDemo extends JFXApp3:
  ...
  override def start(): Unit =
    stage = new JFXApp3.PrimaryStage:
      ...
      val availableCards1: List[Node] = animalCards.take(2).map(c => AnimalCardView(c, 90, 140))
      val completedCards1: List[Node] = animalCards.drop(2).map(c => AnimalCardView(c, 55, 85))
      val playerArea1 = PlayerAreaView(boardView1, availableCards1, completedCards1, "Player 1")

      // player 2
      val board2 = PersonalBoard(SideA)
      val boardView2 = PersonalBoardView(board2, controller)
      val availableCards2: List[Node] = animalCards.take(1).map(c => AnimalCardView(c, 90, 140))
      val completedCards2: List[Nothing] = List.empty
      val playerArea2 = PlayerAreaView(boardView2, availableCards2, completedCards2, "Player 2")
